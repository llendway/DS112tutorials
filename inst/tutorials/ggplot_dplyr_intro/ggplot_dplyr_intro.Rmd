...
Next, watch the video below that walks through some examples in R Studio. Just like with the `ggplot()` material, you can practice along with me by downloading the R Markdown file and working through the problems. If you do that, you will likely get somewhat different results than you see in the video when using the `garden_harvest` data because the data is still changing everyday :)
