...
garden_harvest %>% 
  ...
  arrange(desc(tot_harvest)) %>% 
  ggplot(aes(x = tot_harvest, 
             y = fct_reorder(vegetable, tot_harvest))) +
  geom_col()
```
...
garden_harvest %>% 
  group_by(vegetable) %>% 
  mutate(avg_daily_harvest = mean(weight)) %>% 
  select(avg_daily_harvest, everything())
```
...
garden_harvest %>% 
  ...
  ggplot(aes(x = number_days, 
             y = fct_reorder(vegetable, number_days),
             size = tot_harvest_lb)) +
  geom_point() +
  labs(title = "How many days was each vegetable harvested?",
       x = "",
       y = "",
       size = "Harvest weight (lb)") +
